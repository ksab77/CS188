...
class Perceptron(object):
    def __init__(self, dimensions):
        ...
        "*** YOUR CODE HERE ***"
        self.percepInstance = np.zeros(dimensions)
    ...
    def predict(self, x):
        ...
        """
        "*** YOUR CODE HERE ***"
        prediction = np.dot(self.percepInstance, x)
        #if (type(prediction) == np.ndarray):
        #    prediction = np.all([(pred >= 0) for pred in prediction])
        #else:
        if (prediction >= 0):
            return 1
        else:
    ...
    def update(self, x, y):
        ...
        if (self.predict(x) == y):
            ...
            result = self.percepInstance
            self.percepInstance = result + (y * x)
            return True
    ...
    def train(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        train_result = [self.update(x, y) for x, y in self.get_data_and_monitor(self)]
        #for x, y in self.get_data_and_monitor(self):
        #    self.update(x, y)

        if (np.any(train_result) == True):
            self.train()
