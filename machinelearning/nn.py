...
class MatrixVectorAdd(FunctionNode):
    ...
    def backward(inputs, gradient):
        "*** YOUR CODE HERE ***"
        #return [np.ones((len(inputs[0]), len(inputs[0][0]))) * gradient, np.ones(len(inputs[1])) * gradient]
...
class ReLU(FunctionNode):
    ...
    def backward(inputs, gradient):
        "*** YOUR CODE HERE ***"
        der = np.minimum(np.maximum(inputs, np.zeros_like(inputs)), np.ones(np.array(inputs).shape))
        print (gradient)
        print (inputs)
        return gradient * der
...
class SquareLoss(FunctionNode):
    ...
    def backward(inputs, gradient):
        ...
        return [gradient * sub/len(sub.flat), -gradient * sub/len(sub.flat)]
