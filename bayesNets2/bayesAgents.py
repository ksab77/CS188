...
def fillObsCPT(bayesNet, gameState):
    ...
    possibleHouses = gameState.getPossibleHouses()
    "*** YOUR CODE HERE ***"
    for housePos in gameState.getPossibleHouses():
        for obsPos in gameState.getHouseWalls(housePos):
            obsVar = OBS_VAR_TEMPLATE % obsPos
            ...
            for assignment in obsFactor.getAllPossibleAssignmentDicts():
                ...
                blueProb = 0
                noneProb = 0
                houseIndex = possibleHouses.index(housePos)
                if houseIndex == 0:
                    houseIndex = 2
                elif houseIndex == 1:
                    houseIndex = 0
                ...
                if assignment[FOOD_HOUSE_VAR] == HOUSE_VALS[houseIndex]:
                    redProb = PROB_FOOD_RED
                    blueProb = 1 - PROB_FOOD_RED
                    noneProb = 0
                elif assignment[GHOST_HOUSE_VAR] == HOUSE_VALS[houseIndex]:
                    redProb = PROB_GHOST_RED
                    blueProb = 1 - PROB_GHOST_RED
                    noneProb = 0
                else:
                    noneProb = 1

                if assignment[obsVar] == RED_OBS_VAL:
                    obsFactor.setProbability(assignment, redProb)
                elif assignment[obsVar] == BLUE_OBS_VAL:
                    obsFactor.setProbability(assignment, blueProb)
                else:
                    obsFactor.setProbability(assignment, noneProb)
...
def getMostLikelyFoodHousePosition(evidence, bayesNet, eliminationOrder):
    ...
    """
    "*** YOUR CODE HERE ***"
    newProb = inference.inferenceByVariableElimination(bayesNet, FOOD_HOUSE_VAR, evidence, eliminationOrder)
    maxProb = 0
    maxVal = None
    for assignment in newProb.getAllPossibleAssignmentDicts():
        if (newProb.getProbability(assignment) > maxProb):
            maxProb = newProb.getProbability(assignment)
            maxVal = assignment[FOOD_HOUSE_VAR]

    maxDict = {FOOD_HOUSE_VAR: maxVal}
    return maxDict
...
class VPIAgent(BayesAgent):
    ...
    def computeEnterValues(self, evidence, eliminationOrder):
        ...
        *Do not* take into account the "time elapsed" cost of traveling to each
        ...
        leftDict = {FOOD_HOUSE_VAR: TOP_LEFT_VAL, GHOST_HOUSE_VAR: TOP_RIGHT_VAL}
        leftDict.update(evidence)
        rightDict = {FOOD_HOUSE_VAR: TOP_RIGHT_VAL, GHOST_HOUSE_VAR: TOP_LEFT_VAL}
        rightDict.update(evidence)
        "*** YOUR CODE HERE ***"
        newProb = inference.inferenceByVariableElimination(self.bayesNet, [FOOD_HOUSE_VAR, GHOST_HOUSE_VAR], evidence, eliminationOrder)
        for assignment in newProb.getAllPossibleAssignmentDicts():
            if (assignment == leftDict):
                leftExpectedValue += newProb.getProbability(assignment) * WON_GAME_REWARD
                rightExpectedValue += newProb.getProbability(assignment) * GHOST_COLLISION_REWARD
            elif (assignment == rightDict):
                leftExpectedValue += newProb.getProbability(assignment) * GHOST_COLLISION_REWARD
                rightExpectedValue += newProb.getProbability(assignment) * WON_GAME_REWARD
    ...
    def computeExploreValue(self, evidence, enterEliminationOrder):
        ...
        "*** YOUR CODE HERE ***"
        newProb = self.getExplorationProbsAndOutcomes(evidence)

        for prob, outcome in newProb:
            expectedValue = expectedValue + prob * max(self.computeEnterValues(outcome, enterEliminationOrder))
