...
def joinFactors(factors):
    ...
    for factor in factors:
        for var in factor.unconditionedVariables():
            setOfUnconditioned.add(var)
        for var in factor.conditionedVariables():
            setOfConditioned.add(var)

    setOfConditioned.difference_update(setOfUnconditioned)
    
    joinedFactor = Factor(setOfUnconditioned, setOfConditioned, factors[0].variableDomainsDict())

    for assignment in joinedFactor.getAllPossibleAssignmentDicts():
        jointProb = 1
        for factor in factors:
            jointProb *= factor.getProbability(assignment)
        joinedFactor.setProbability(assignment, jointProb)
    ...
    def eliminate(factor, eliminationVariable):
        ...
        for newAssignment in newFactor.getAllPossibleAssignmentDicts():
            ...
            for var in factor.variableDomainsDict()[eliminationVariable]:
                tempAssignment = newAssignment
                tempAssignment[eliminationVariable] = var
                jointProb += factor.getProbability(tempAssignment)

            newFactor.setProbability(newAssignment, jointProb)
        return newFactor
...
def normalize(factor):
    ...
    newFactor = Factor(setOfUnconditioned, setOfConditioned, factor.variableDomainsDict())
