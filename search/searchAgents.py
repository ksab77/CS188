...
class CornersProblem(search.SearchProblem):
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            nextx, nexty = int(x + dx), int(y + dy)
            if not self.walls[nextx][nexty]:
                if (((nextx, nexty) in self.corners) & ((nextx, nexty) not in state[1])):
...
def cornersHeuristic(state, problem):
    ...
    closewalls = 0
    pos = state[0]
    nearby = ((pos[0] - 1, pos[1]), (pos[0] + 1, pos[1]), (pos[0], pos[1] - 1), (pos[0], pos[1] + 1))
    start = problem.getStartState()[0]
    #for near in nearby:
        #if near in walls:
            #closewalls += 1
    cornerdistance += closewalls
    for visited in state[1]:
        corners.remove(visited)
    for corner in corners:
        #distance = abs(((pos[0]-corner[0])*(start[1]-corner[1])) - ((start[0]-corner[0])*(pos[1]-corner[1])))
        distance = (abs(pos[0] - corner[0]) + abs(pos[1] - corner[1]))
        cornerdistance += distance/1.5
    if pos in problem.corners:
        return 0
...
def foodHeuristic(state, problem):
    ...
    start, startfood = problem.getStartState()
    fooddistance = 0
    closestfood = 999
    ...
    for food in foodpos:
        fooddistance += abs(((position[0]-food[0])*(start[1]-food[1])) - ((start[0]-food[0])*(position[1]-food[1])))/100
    if (position in startfood.asList()):
        fooddistance = 0
    print fooddistance
    return fooddistance
